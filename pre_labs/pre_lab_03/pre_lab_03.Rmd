...
**Answer** One or more parsing issues, see 'prbolems()' for details
...
**Answer** amount: dbl, name: chr, id: dbl
```{r}
...
**Answer**: 135,073; yes it is the same as the number for forgiveness_amount and forgiveness_date run separately; I can conclude that they are the same records.
...
**Answer** zero loans in October 2020
```{r}
ppp_maryland_loans %>%
...
**Answer** largest loan: \$10 million; smallest loan: \$6; \$10 million is the highest amount a person/business could get from a PPP loan, so I'm not surprised by the largest loan number. It seems odd that someone would apply for a PPP loan and only ask for/receive \$6 though because that is not enough money to pay employees or keep a business running in any capacity.
```{r}
...
**Answer** yes the fields with amounts match for both loans under \$100.
...
**Answer** The ID column is named weird; the street address column has a space instead of an underscore, the values in the zip column are not uniform
...
**Answer** Error: Problem with `summarise()` column `total_amount`. ℹ `total_amount = sum(amount)`. x invalid 'type' (character) of argument Run `rlang::last_error()` to see where the error occurred. The error means that the column "amount" cannot be summed up because the values are characters, not numbers.
...
**Answer** 2 duplicates; business name: Stuart Jones LLC
