...
```{r}
latest_date_wvu_crime <- latest_wvu_crime_logs %>% slice(1)
thirty_days_ago <- wvu_crime_logs %>% filter(title == "VEHICLE ACCIDENT" & date >= latest_date_wvu_crime$date - 30)
```


```{r}
sentence <- paste("Since ", format(latest_date_wvu_crime$date, format="%B %d, %Y"), ", ", nrow(thirty_days_ago), " ", tolower(latest_date_wvu_crime$title), "s have occurred in the last 30 days.", sep="")

#, " ", direction, " by ", changetext, " percent compared to the week before", sep="")
sentence
```
